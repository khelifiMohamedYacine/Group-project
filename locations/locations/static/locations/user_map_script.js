...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    data.forEach((location) => {
      ...
      const markerElement = new maplibregl.Marker({ color: markerColor })
      ...
      markerElement.getElement().addEventListener("click", function (event) {
        ...
        card.innerHTML = `
                    <div class="card-body">
                        <h5 class="card-title">${location.location_name} (ID: ${
          location.locID
        })</h5>
                        ${taskDetails}
                        <button class="btn ${
                          ...
                          location.status === "completed"
                            ...
                            ? "btn-success"
                            : "btn-primary"
                        } w-100 mt-2 check-in-btn" data-locid="${
          location.locID
        }" 
                        ${
                          location.status === "locked" ||
    ...
    document.addEventListener("click", function (event) {
      if (
        ...
        !event.target.classList.contains("check-in-btn")
      ) {
        card.style.display = "none";
        activeMarker = null;
      }
...
document
  .getElementById("bypassCheckInBtn")
